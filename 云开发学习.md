...
## 6.11 sitemap.json的使用

- 这个页面是配置那些文件可以被索引（搜索），那些页面不可以被索引

- 下面这个文件代表只有 pages/player/player 页面并且带有musicId和index参数的页面可以被索引到，其他的页面都无法被索引
...
1. 可以在开发者工具右侧 >> 按钮点击上传，首次推荐版本为1.0.0
2. 注意选择的类目要符合规定，<https://developers.weixin.qq.com/miniprogram/product/material/#%E4%B8%AA%E4%BA%BA%E4%B8%BB%E4%BD%93%E5%B0%8F%E7%A8%8B%E5%BA%8F%E5%BC%80%E6%94%BE%E7%9A%84%E6%9C%8D%E5%8A%A1%E7%B1%BB%E7%9B%AE>
3. 点击提交审核即可
...
- 使用vue制作后台管理项目
- 前台使用vue加上现成的模板 vue-admin-template
- 后台使用Koa2
- 前后台分离的架构
...
- 首先下载一个vue-admin-template的模板

  ```
  git clone https://github.com/PanJiaChen/vue-admin-template.git
  ```

- 在npm i 安装项目的依赖之后就可以 npm run dev 运行项目

- 删除原有view文件下除了login和404以外的页面，按照自己的需求新建需要的页面，并配置路由。

- 路由前期基本配置 （踩坑注意，父路由也就是有children的路由的路径有  /  例如 /login ）,子路由的路径path 没有 / 也就是 path: 'children'
...
### 7.2.1获取accessToken的操作

- 由于许多请求都需要这个操作，因此可以封装为一个文件

  ```javascript
  const rp = require('request-promise')
  // 定义appid
  const APPID  ='wxc0aee5e549a2a8ea'
  // 定义秘钥
  const APPSECRET = '6f37ef8eb01944c1bba4f9216a526993'
  // 小程序官方获取access_token的接口
...
- 有获取的方法和存入的方法，存入的方法会调用小程序的接口获取accessToken，并存入一个文件中，如果调用失败就重新调用

- 获取的方法是读取刚才存入的文件，如果没有这个文件就调用一下存入的方法创建文件

- 获取到后可以判断当前时间与存储的事件相隔是否超过两小时，若超过两小时，说明token是失效的，那么重新获取token

- 如果获取token失败，那么肯定是因为这个存储数据的文件没创建，那么调用存入的方法创建这个文件即可

- 设置一个定时器，因为token每两个小时就会失效，因此隔两小时调用一次即可

- 最后导出这个获取token的方法，这个方法的返回值就是获取的token
...
- 导入定义的其他路由，并使用router.use("路径名"，子路由)的方式router.use('/playlist', playlist.routes())  来注册子路由

- 这时可以通过 /playlist/list 得到子路由的数据

- 只有最后app.use（）才是真正应用，之前的router.use（）都是给router实例增加新的属性
  ...
  // 这相当于是父子嵌套，例如请求 /playlist/list就会得到playlist内部的路由
...
- 找到请求所需的参数，accessToken,可在之前定义的函数中获取，env就是小程序的app.js中的env，name是你要调用的云函数名称

- 之后使用promise-request发送post请求，携带数据 $url对应云函数端用tcb-router设定的路由名称即可

  ```javascript
  const Router = require('koa-router')
  const rp = require('request-promise')
  const router = new Router()
  const ACCESS_TOKEN = require('../utils/getAccessToken')
  
  router.get('/list', async (ctx, next) => {
    // POST https://api.weixin.qq.com/tcb/invokecloudfunction?access_token=ACCESS_TOKEN&env=ENV&name=FUNCTION_NAME
    // 获取之前获得token
    // 这是小程序段的env，在app.js中可以找到
    const ENV = 'zy-cloud-rczke'
    const TOKEN = await ACCESS_TOKEN()
    // 获取云函数的小程序接口
    let url = `https://api.weixin.qq.com/tcb/invokecloudfunction?access_token=${TOKEN}&env=${ENV}&name=music`
    console.log(url, 'url')
    var options = {
      method: 'POST',
      uri: url,
...
- 简单的配置（在app.js中配置）
  ...
  app.use(cors({
    origin: ['http://localhost:9528'], // 允许的路径
    credentials: true // 证书
  }))
  ```
...
```javascript
    getMusciList() {
      ...
      let res = fetchList(params).then(res => {
        this.loading = false;
        this.playlist = this.playlist.concat(res.data);
        console.log(res.data.length, "数据");
        // 说明剩下的数据不足取一次，也就是取完了
...
- 封装的请求,后台可通过 ctx.request.query获取请求传递的参数
...
- 封装scroll插件，具体作用为，传递一个函数，会监听鼠标滚动的事件来执行这个函数
...
#### 主要的区别是get请求携带参数用params,post使用data来携带
...
- 推荐使用以下方式
- 设置路由，不在后面写入：id等参数
- 不用后面加：id的方式参数使用query来传递，不会显示在url上，获取也是$route.query.id即可

```
      {
        path: "edit",
        meta: { title: "歌单编辑", icon: "table" },
        hidden: true,
...
- 在封装的请求函数中，写入如下对象,当调用loading的open方法时会显示加载中，只有调用了close把状态位重新置空才可以调用下一次loading方法
...
- 适用场景演示, 在请求和响应拦截器中分别调用open和close即可
